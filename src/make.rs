...
pub fn build_make(directory: PathBuf, noinstall: bool) {
	...
	for line in reader.lines() {
		if let Ok(mut content) = line {
			...
			if content.contains("CC") || content.contains("LD") || content.contains("LINK") || content.contains("AR") || content.contains("checking"){
...
pub fn clean(directory: PathBuf) {
	...
	if waiter.success() {
		...
		println!("If you want, you can try to clean manually by running \"make clean\" in the project root.")
	}
}
